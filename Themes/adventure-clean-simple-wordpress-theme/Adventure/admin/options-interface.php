...
function optionsframework_fields() {
	...
	foreach ($options as $value) {
		...
		// Wrap all options
		if ( ($value['type'] != "heading") && $value['type'] != "optiongroup" && $value['type'] != "endoptiongroup" && ($value['type'] != "info") ) {
		...
		case 'textarea':
			...
			$val = stripslashes( $val );
			$editor = $value['class'] == 'editor' ? ' editor' : '';
			
			$output .= '<textarea id="' . esc_attr( $value['id'] ) . '" class="of-input' . $editor . '" name="' . esc_attr( $option_name . '[' . $value['id'] . ']' ) . '" cols="'. esc_attr( $cols ) . '" rows="8">' . esc_textarea( $val ) . '</textarea>';
		break;
		...
		case 'include':
			foreach ($value['options'] as $key => $option) {
				...
				$heights = array(
						'simple-slider'		=> 600,
						'nivo-slider' 		=> 695,
						'nivo-thumb-slider' => 800,
						'simple-content'	=> 535,
						'3d-slider'			=> 660,
						'accordion-slider'	=> 695,
						'video-homepage'	=> 760,
						'grid-homepage'		=> 760,
						'static-homepage'	=> 500,
						'cycle-slider'		=> 560
					);
				$output .= '<div id="' . esc_attr( $id ) . '" class="include hidden ' . $key . '"><iframe id="' . $key . '-frame" width="1040" height="' . $heights[$key] . '" src="' . get_home_url() . '?slider=' . $key . '" scrolling="no"></iframe></div>';
			}
		break;
